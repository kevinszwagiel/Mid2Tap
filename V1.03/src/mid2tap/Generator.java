...
public class Generator {
    ...
    // This is bad practice. Should probably change this
    ...
    public static LinkedList<Note> parseMidi(File eofFile) {
        ...
        try {
            ...
            // Check if the file is from EOF (crazy notes are different)
            ...
            for (Track track :  sequence.getTracks()) {
                ...
                for (int i=0; i < track.size(); i++) { 
                    ...
                    if (message instanceof ShortMessage) {
                        ...
                        notes.add(note);
                        //System.out.print("Note Value: " + note.getNoteValue());
                        //System.out.print(", Quarter Time: " + note.getQuarterTime());
                        //System.out.print(", Real Time: " + note.getRealTime());
                        //System.out.println(", StartEnd: " + note.getStartEnd());
                    }
                }
            }
            
            int eofCheck = isEof(eofFile);
            ...
            if(eofCheck == 0) {
                ...
                // If there are 3 or less unique note values.
            ...
            // EOF MIDI file. Currently only checks extreme difficulty
            else if(eofCheck == 1) {
                LinkedList<Integer> extremeNotes = new LinkedList<>();
                LinkedList<Integer> extremeTracker = new LinkedList<>();

                for(int i = 0; i < noteTracker.size(); i++) {
                    int tempNote = noteTracker.get(i);

                    // Only get note values from the extreme difficulty
                    if(tempNote >= 95 && tempNote <= 100) {
                        if(!extremeTracker.contains(tempNote)) {
                            extremeTracker.add(tempNote);
                        }
                    }
                }

                if(extremeTracker.size() <= 3 && extremeTracker.size() > 0) {
                    int[] extremeConverter = new int[3];
                    extremeConverter[0] = (int) extremeTracker.get(0);
                    extremeConverter[1] = (int) extremeTracker.get(1);
                    extremeConverter[2] = (int) extremeTracker.get(2);
                    
                    Arrays.sort(extremeConverter);
                    
                    // Extreme notes
                    for(int i = 0; i < notes.size(); i++) {
                        Note tempNote = notes.get(i);
                        
                        if(tempNote.getNoteValue() == extremeConverter[0]){
                            tempNote.setNoteValue(60);
                            notes.set(i, tempNote);
                        }

                        else if(tempNote.getNoteValue() == extremeConverter[1]) {
                            tempNote.setNoteValue(62);
                            notes.set(i, tempNote);
                        }
    ...
    /**
     * Determine whether the MIDI file is one created in EOF. This is done by
     * reading the first 256 bytes and searching for the hex value of "PART GUITAR".
     * The space is actually a DC4 character.
     * 
     * @param eofFile The MIDI file specified by the user
     * @return An integer value. 1 = true, 0 = false, -1 = error
     * @exception IOException Caught if the file cannot be read
     */
    public static int isEof(File eofFile) {
        
        try {
            // Read the first 256 bytes of the MIDI. This should contain
            // data as to whether it is an EOF file or not
            byte[] b = new byte[256];
            String[] byteStrings = new String [256];
            String s = "";
            FileInputStream reader = new FileInputStream(eofFile);
            ...
            // Hex for "PART GUITAR". The space is actually a DC4. This tends to
            // be exclusive to EOF MIDI files
            if(s.contains("5041525420475549544152")) {
                return 1;
            }
